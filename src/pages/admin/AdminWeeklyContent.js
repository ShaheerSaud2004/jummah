...
const AdminWeeklyContent = () => {
  ...
  const loadContent = () => {
    const data = getData('weeklyContent');
    setContent(data);
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (editingId) {
      updateItem('weeklyContent', editingId, formData);
    } else {
      addItem('weeklyContent', formData);
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <h1 className="text-4xl font-serif font-bold text-rutgers-red">Manage Weekly Content</h1>
          </div>
          <button
            onClick={() => {
              resetForm();
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <select
                  ...
                  required
                >
                  <option value="ayah">Qur'an Ayah</option>
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rutgers-red text-right font-arabic text-lg"
                  required
                />
              </div>
              {formData.type === 'dua' && (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    <span className="bg-rutgers-red text-white px-3 py-1 rounded-full text-sm font-semibold">
                      {item.type === 'ayah' ? 'Qur\'an Ayah' : 'Du\'a'}
                    </span>
                    {item.reference && (
                      <span className="text-sm text-gray-600">{item.reference}</span>
                    )}
                  </div>
                  <div className="text-arabic text-rutgers-red text-xl mb-3 text-right">
                    {item.arabic}
                  </div>
...
export default AdminWeeklyContent;
